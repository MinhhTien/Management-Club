...
public interface ChallengeRepository extends JpaRepository<Challenge, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT * from challenge WHERE id = ?1 AND status = 'active'")
    Challenge findChallengeById(Integer id);

    @Query(nativeQuery = true, value = "SELECT * from challenge WHERE title = ?1 AND status = 'active'")
    Challenge findChallengeByTitle(String title);

    @Query(nativeQuery = true, value = "SELECT * from challenge WHERE register_url = ?1 AND status = 'active'")
