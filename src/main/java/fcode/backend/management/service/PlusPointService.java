...
public class PlusPointService {
    ...
    public Response<List<PlusPointDTO>> getByMemberIdBetweenTime(Integer memberId, Date date1, Date date2) {
        logger.info("{}{}{}{}{}{}", GET_PLUS_POINT, memberId, "from", date1, "to", date2);
        ...
        List<PlusPointDTO> plusPointDTOList = plusPointRepository.getByMemberIdAndStatusBetweenTime(memberId, date1, date2, Status.ACTIVE.toString())
                .stream().map(plusPoint -> {
                    PlusPointDTO plusPointDTO = modelMapper.map(plusPoint, PlusPointDTO.class);
                    plusPointDTO.setMemberId(plusPoint.getMember().getId());
                    return plusPointDTO;
